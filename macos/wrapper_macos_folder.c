...
int main (int argc, char* argv[])
{
    ...
    // Generate IDE exe path (Resources/bin/<ide_exe>)
    strncpy(coqide_path, bin_folder, sizeof(coqide_path)-1);
    coqide_path[sizeof(coqide_path)-1] = '\0';
    strncat(coqide_path, "/", sizeof(coqide_path)-strlen(coqide_path)-1);
    strncat(coqide_path, ide_exe, sizeof(coqide_path)-strlen(coqide_path)-1);
