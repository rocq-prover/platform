...
TEST_FILES[coq-extructures~9.0.1+preview~2025.08]='tests/tutorial.v'
...
PATCH_CMDS[coq-hott~9.0.1+preview~2025.08]='/^Require Import$/ {print "From HoTT "$0; next}'
...
TEST_FILES[coq-mathcomp-algebra~9.0.1+preview~2025.08]='mathcomp/algebra/finalg.v'
...
TEST_FILES[coq-mathcomp-character~9.0.1+preview~2025.08]='character/all_character.v'
TEST_FILES[coq-mathcomp-field]='mathcomp/field/finfield.v'
TEST_FILES[coq-mathcomp-field~9.0.1+preview~2025.08]='field/finfield.v'
TEST_FILES[coq-mathcomp-fingroup]='mathcomp/fingroup/quotient.v'
TEST_FILES[coq-mathcomp-fingroup~9.0.1+preview~2025.08]='fingroup/quotient.v'
...
TEST_FILES[coq-mathcomp-solvable~9.0.1+preview~2025.08]='solvable/abelian.v'
TEST_FILES[coq-mathcomp-ssreflect]='mathcomp/ssreflect/ssrbool.v'
TEST_FILES[coq-mathcomp-ssreflect~9.0.1+preview~2025.08]='ssreflect/ssrbool.v'
...
TEST_FILES[coq-mathcomp-word~9.0.1+preview~2025.08]='../../test_files/coq-mathcomp-word/test.v'
...
TEST_FILES[coq-riscv~9.0.1+preview~2025.08]='src/riscv/Examples/Fib.v'
...
TEST_FILES[coq-simple-io~9.0.1+preview~2025.08]='test/example/main.v test/extraction/main.v'
...
do
  ...
  then
    ...
    do
      base_src_name=${file##*/}
      dest_name=${base_src_name/-/_}
      src_path="smoke-test-kit/${package}-src/$file"

      if [ ! -f "$src_path" ]; then
        echo "Source file '$src_path' not found; trying to auto-detect '$base_src_name' in ${package}-src"
        alt_path="$(find "smoke-test-kit/${package}-src" -type f -name "$base_src_name" | head -n 1 || true)"
        if [ -n "$alt_path" ] && [ -f "$alt_path" ]; then
          src_path="$alt_path"
          echo "Auto-detected path: $src_path"
        else
          echo "Error: could not locate '$base_src_name' in sources of ${package} (${packagefull})"
          exit 1
        fi
      fi

      cp "$src_path" "smoke-test-kit/${package}/${dest_name}"
      patch_file "smoke-test-kit/${package}/${dest_name}" "${patches}"

      if [ -z "$commands" ]; then
        echo "run_test ${package}/${dest_name} \"$options\"" >> $smoke_script
        echo "CALL :run_test ${package}/${dest_name} \"${options//\$COQLIB/%COQLIB%}\""$'\r' >> $smoke_batch
      fi
    done
